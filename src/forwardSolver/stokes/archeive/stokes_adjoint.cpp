...
stokesSolver::stokesSolver(int level_) : level(level_){
	...
	apply_boundary_condition();
 	...
 	VecCreate(MPI_COMM_SELF, &adjointRhs);
	VecSetSizes(adjointRhs, PETSC_DECIDE, 2*num_node_P1isoP2+num_node_P1);
	VecSetFromOptions(adjointRhs);
	VecSet(adjointRhs, 0.0);
	VecAssemblyBegin(adjointRhs);
	VecAssemblyEnd(adjointRhs);
...
double stokesSolver::fsource_i(double x, double y, int i){
    double output = u[i]/(i+1.0)/(i+1.0)*std::sin(2.*(i+1.0)*M_PI*x)*std::sin(2.*(i+1.0)*M_PI*y);		
	return 100*output;	
}
...
void stokesSolver::load_vector_element_i(double x1[], double x2[], double v[], int i){
	...
	double y = (x2[0] + x2[1] + x2[2])/3;
	for (int i = 0; i < 3; i++){
		v[i] = 1.0/6.0*fsource_i(x, y, i)*Jdet;
	}
}
...
void stokesSolver::load_vector_i(int i, double load[]){
	...
	for (int i = 0; i < num_P1isoP2_element; i++){
		...
		for (int k = 0; k < 3; k++){
			load[triangles[k][i]] = v_element[k];
			load[num_node_P1isoP2+triangles[k][i]] = v_element[k];
		}
	}
};
...
void stokesSolver::apply_boundary_condition(){
	...
	MatZeroRows(finalMatrix, 2*4*division_P1isoP2, Bs.get(), 1.0, workspace, finalRhs);
	// MatZeroRowsColumns(finalMatrix, 2*4*division_P1isoP2, Bs.get(), 1.0, workspace, finalRhs);
};
...
void stokesSolver::apply_homo_boundary_condition(){
	...
	VecAssemblyBegin(workspace);
	VecAssemblyEnd(workspace);
	MatZeroRows(finalMatrix, 2*4*division_P1isoP2, Bs.get(), 1.0, workspace, adjointRhs);
	// MatZeroRowsColumns(finalMatrix, 2*4*division_P1isoP2, Bs.get(), 1.0, workspace, finalRhs);
};
...
void stokesSolver::pointObsMatrix(double obs[][4], int size){
	...
	MatSetSizes(state2obsMatrix, PETSC_DECIDE, PETSC_DECIDE, 2*size, 2*num_node_P1isoP2+num_node_P1);
	...
	for (int i = 0; i < size; i++){
		...
		MatSetValue(state2obsMatrix, i, triangle[0], 1-epsilon-eta, INSERT_VALUES);
		MatSetValue(state2obsMatrix, i, triangle[1], epsilon, INSERT_VALUES);
		MatSetValue(state2obsMatrix, i, triangle[2], eta, INSERT_VALUES);

		MatSetValue(state2obsMatrix, i+size, num_node_P1isoP2+triangle[0], 1-epsilon-eta, INSERT_VALUES);
		MatSetValue(state2obsMatrix, i+size, num_node_P1isoP2+triangle[1], epsilon, INSERT_VALUES);
		MatSetValue(state2obsMatrix, i+size, num_node_P1isoP2+triangle[2], eta, INSERT_VALUES);
   	}
...
void stokesSolver::controlEqn(double grad[]){
	...
	double workspace;
	double load[2*num_node_P1isoP2];
	std::cout << "gradient: ";
	for (int i = 0; i < 10; ++i){
		load_vector_i(i, load);
		workspace = 0;
		for (int j = 0; j < 2*num_node_P1isoP2; ++j){
...
void stokesSolver::WuuApply(Vec direction, int size){
	...
	MatMult(pointObsMatrix, direction, workspace);
	MatMultTranspose(pointObsMatrix, workspace, direction);
}

Vec stokesSolver::CApply(Vec direction, int size, bool T){
	Mat C;
	MatCreate(MPI_COMM_SELF, &C);
	MatSetSizes(C, PETSC_DECIDE, PETSC_DECIDE, 2*num_node_P1isoP2+num_node_P1, 10);
	MatSetFromOptions(C);
	...
	for (int i = 0; i < 10; ++i){
		load_vector_i(i, load);
		for (int j = 0; j < 2*num_node_P1isoP2+num_node_P1; ++j){
	...
	Vec workspace;
	if (T == 0){
		VecSetSizes(workspace, PETSC_DECIDE, 2*num_node_P1isoP2+num_node_P1);
		VecSet(workspace, 0.0);
		VecAssemblyBegin(workspace);
		VecAssemblyEnd(workspace);
...
void stokesSolver::ASolve(){

}

void stokesSolver::RApply(){
...
int main(int argc, char* argv[]){
	...
	// }
	// std::cout << "obs generation" << std::endl;
	// write2txt(obs, obsNumPerRow*obsNumPerRow, obs_file);
	txt2read(obs, obsNumPerRow*obsNumPerRow, obs_file);


	stokesSolver testSolver(levels);
    for (int i = 0; i < 10; ++i){
    	testSolver.u[i] = testSolver.distribution(testSolver.generator);
    }
	testSolver.pointObsMatrix(obs, obsNumPerRow*obsNumPerRow);

    double grad[10];
    testSolver.forwardEqn();
    testSolver.adjointEqn(obsNumPerRow*obsNumPerRow, obs);
    testSolver.controlEqn(grad);
	///////////Sampling///////////////////
