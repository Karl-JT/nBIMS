...
void mainSolver(const vector<adouble>& betaML, adouble& J){
	...
	for (int i = 1; i < yCoordinate.size()-1; i++) {
		J = J + 1e5*pow((xVelocity[i]-dnsData.U[i]*turbProp.frictionVelocity)*(yCoordinate[i]-yCoordinate[i-1]), 2) + pow(10, 3)*pow((betaML[i]-1)*(yCoordinate[i]-yCoordinate[i-1]), 2);
	}
