...
public:
	...
	MLMCMC_Bi_Uniform(MPI_Comm PSubComm_, int levels_, int sampleSize_, int rank_, int a_, double noiseVariance_, int beta_=1): PSubComm(PSubComm_), levels(levels_), sampleSize(sampleSize_), rank(rank_), a(a_), noiseVariance(noiseVariance_), beta(beta_){
		L = new double[levels+1];
		solvers.resize(levels+1);
		for(int i = 0; i < levels+1; ++i){
			solvers[i] = std::make_shared<solverType>(i, sampleSize, noiseVariance);
		}
		generator.seed(rank_*13.0);
		...
		if (subSize > 1){
			...
			for (int j = 1; j < levels-subRank; ++j){
			  ...
			  //   	dummies[0]->readData(0, M);
					...
					// MLChain_Uni_ijUpper<samplerType, solverType> chainijUpper(M, sampleSize, dummies[0].get(), solvers[subRank].get(), solvers[j].get(), solvers[j-1].get());
