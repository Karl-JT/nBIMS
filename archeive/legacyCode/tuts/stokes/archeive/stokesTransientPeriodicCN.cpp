...
stokesSolver::stokesSolver(int level_, int num_term_, double noiseVariance_) : level(level_), num_term(num_term_), noiseVariance(noiseVariance_) {
	...
	int steps = floor(sqrt(2.0*(std::pow(2.0, level_))))+1;
	if (steps % 2 != 0){
		steps += 1;
	}
	delta_t = 1.0/steps;
...
void stokesSolver::Q1isoQ2Cond(){
	...
	MatScale(sysMatrix, 1.0/2.0);

	MatDestroy(&workspace);

	MatCreateSubMatrix(sysMatrix, isrow, iscol, MAT_INITIAL_MATRIX, &stiffMatrix);
...
void stokesSolver::system_rhs(){
	...
	MatView(stiffMatrix, PETSC_VIEWER_STDOUT_WORLD);	
	MatMultAdd(stiffMatrix, states, finalRhs, finalRhs);
...
void stokesSolver::linear_system_setup(){
	...
	system_matrix();
	Q1isoQ2Cond();
	MatDuplicate(stiffMatrix, MAT_COPY_VALUES, &finalMatrix);
