...
int main(int argc, char* argv[])
{
	...
	switch (confData.task){
        ...
        {
            NSE2dDirSolver singleForwardSolver(PETSC_COMM_SELF, confData.levels, confData.num_term,confData.noiseVariance);
        ...
        case 2: //MLMCMC estimation
        {
            ...
            if (confData.parallelChain == 1){
                ...
                MLMCMC_Bi_Uniform<pCN<NSE2dDirSolver>, NSE2dDirSolver> MLMCMCSolver(PSubComm, confData.levels, 1, color, confData.a, confData.noiseVariance, confData.randomSeed, 1.0);
                ...
                MLMCMC_Bi_Uniform<pCN<NSE2dDirSolver>, NSE2dDirSolver> MLMCMCSolver(PETSC_COMM_SELF, confData.levels, 1, rank, confData.a, confData.noiseVariance, confData.randomSeed, 1.0);
                output = MLMCMCSolver.mlmcmcRun();			
                ...
                for (int j=0; j<meshsize; ++j){
                    ...
                    for (int n=0; n<ngp; n++){
                        for (int m=0; m<ngp; m++){
                            ...
                            // std::cout << uyx[n*ngp+m] << " " << pow(uyx[n*ngp+m],2) <<" " << x[m] << " " << y[n] << " " << quadw[m]*quadw[n] << " "  << std::endl;
