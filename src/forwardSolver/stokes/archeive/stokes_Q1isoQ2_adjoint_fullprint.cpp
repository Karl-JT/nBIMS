...
void plain_mcmc(stokesSolver *solver, int max_samples, double obs[][4], int obs_size, double mean[], int coef_size, int rank)
{
	std::string L0_file = "L0Chain_";
	L0_file.append(std::to_string(rank));
...
void ml_mcmc(int levels, std::vector<std::shared_ptr<stokesSolver>> solvers, double obs[][4], double out[], int size, int a, int coef_size, int rank)
{
    std::uniform_real_distribution<double> uni_dist(0.0, 1.0);
    ...
    plain_mcmc(solvers[0].get(), M, obs, size, mean, coef_size, rank);
    ...
    for (int i = 0; i < levels; i++){
		std::string LUpper_file = "L";
        LUpper_file.append(std::to_string(i+1));
        LUpper_file.append("UpperChain_");
		LUpper_file.append(std::to_string(rank));

		std::string LLower_file = "L";
        LLower_file.append(std::to_string(i+1));
        LLower_file.append("LowerChain_");
		LLower_file.append(std::to_string(rank));

		std::ofstream upperFile;
		std::ofstream lowerFile;
		upperFile.open(LUpper_file);
		lowerFile.open(LLower_file);
    	...
    	for (int chainNum = 0; chainNum < 2; chainNum++){
            ...
            for (int sampleNum = 0; sampleNum < M; sampleNum++){
    			...
    			if (chainNum == 0){
    				upperFile << u[0] << " " << u[1] << " " << u[2] << " " << u[3] << " " << lnlikelihoodUpper << " " << lnlikelihoodLower; 
    				upperFile << std::endl;
    			} else {
    				lowerFile << u[0] << " " << u[1] << " " << u[2] << " " << u[3] << " " << lnlikelihoodUpper << " " << lnlikelihoodLower; 
    				lowerFile << std::endl;
    			}
    		}
    	}	
...
int main(int argc, char* argv[]){
	...
	if (task == 1){ //Run plain MCMC
		...
		plain_mcmc(samplerSolver, plainMCMC_sample_number, obs, obsNumPerRow*obsNumPerRow, mean, num_term, rank);
	...
	if (task == 2){ //Run MLMCMC
		...
		double out[num_term];
		ml_mcmc(levels, solvers, obs, out, obsNumPerRow*obsNumPerRow, a, num_term, rank);
