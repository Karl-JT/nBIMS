...
def weak_residual(x, xt, sigma, m, xl):
    ...
    res_u =   2.*(nu+nu_t(kl, el, m))* inner( _strain(u), _strain(u_test))* dx \
           ...
           - p* div(u_test)* dx \
           + Cd*(nu+nu_t(kl, el, m))*hbinv* dot(ul-u_ff, tg)* dot(u_test, tg)*ds(3) \
           -  dot( sigma_n(nu+nu_t(kl, el, m), ul), tg) *  dot(u_test, tg)*ds(3) \
           -  dot( sigma_n(nu+nu_t(kl, el, m), u_test), tg ) *  dot(ul - u_ff, tg)*ds(3)
    ...
    res_k =  dot(ul,  grad(k))*k_test* dx \
            ...
            - _chi_inflow(ul)*(nu + nu_t(kl,el,m)/sigma_k)* dot( grad(kl), n)*k_test*ds(3) \
            - _chi_inflow(ul)*(nu + nu_t(kl,el,m)/sigma_k)* dot( grad(k_test), n)*(kl-k_ff)*ds(3) \
            + _chi_inflow(ul)*Cd*(nu + nu_t(kl,el,m)/sigma_k)*hbinv*(kl-k_ff)*k_test*ds(3)
    ...
    res_e =  dot(ul,  grad(e))*e_test* dx \
            ...
            - _chi_inflow(ul)*(nu + nu_t(kl,el,m)/sigma_e)* dot( grad(el), n)*k_test*ds(3) \
            - _chi_inflow(ul)*(nu + nu_t(kl,el,m)/sigma_e)* dot( grad(e_test), n)*(el-e_ff)*ds(3) \
            + _chi_inflow(ul)*Cd*(nu + nu_t(kl,el,m)/sigma_e)*hbinv*(el-e_ff)*e_test*ds(3)
...
def strong_residual(x, xl, m):
       
    u, p, k, e   = split(x)
    ul, pl, kl, el = split(xl)

    res_u = - div(  2.*(nu+nu_t(kl, el, m))*_strain(u) ) +  grad(u)*u +  grad(p)
    res_p =  div( u )
    ...
    res_e = +  dot(ul,  grad(e)) \
            ...
            + C_e2*e*_theta(kl,el) \
            - C_e1* exp(m)*C_mu*k*production(ul)

    return [res_u, res_p, res_k, res_e]


def totalResidual(x, xt, sigma, m, xl):
    return weak_residual(x, xt, sigma, m, xl) + stab(x, xl, xt, m)
