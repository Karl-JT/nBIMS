...
stokesSolver::stokesSolver(int level_, int num_term_, double noiseVariance_) : level(level_), num_term(num_term_), noiseVariance(noiseVariance_) {
	...
	VecDuplicate(finalRhs, &states_n);
	VecDuplicate(finalRhs, &states_r);
	VecZeroEntries(states);
	VecZeroEntries(states_n);
	VecZeroEntries(states_r);
...
void stokesSolver::createAllocations(){
	...
	MatSetSizes(A, PETSC_DECIDE, PETSC_DECIDE, 3*num_node_Q1isoQ2, 3*num_node_Q1isoQ2);
	if (level > 3){
	...
	MatCreate(PETSC_COMM_SELF, &C1);
	MatSetSizes(C1, PETSC_DECIDE, PETSC_DECIDE, 3*num_node_Q1isoQ2, 3*num_node_Q1isoQ2);
	if (level > 3){
		MatSetType(C1, MATSEQAIJCUSPARSE);
	} else {
		MatSetType(C1, MATSEQAIJ);
	}
	// MatSetFromOptions(A1);
	MatSeqAIJSetPreallocation(C1, 24, NULL);
	MatMPIAIJSetPreallocation(C1, 24, NULL, 24, NULL);

	MatCreate(PETSC_COMM_SELF, &C2);
	MatSetSizes(C2, PETSC_DECIDE, PETSC_DECIDE, 3*num_node_Q1isoQ2, 3*num_node_Q1isoQ2);
	if (level > 3){
		MatSetType(C2, MATSEQAIJCUSPARSE);
	} else {
		MatSetType(C2, MATSEQAIJ);
	}
	// MatSetFromOptions(A2);
	MatSeqAIJSetPreallocation(C2, 24, NULL);
	MatMPIAIJSetPreallocation(C2, 24, NULL, 24, NULL);
...
void stokesSolver::c1_matrix_element(double x1[4], double x2[4], double ux[4], double c1[4][4]){
	...
	for (int i = 0; i < 5; i++){
		for (int j = 0; j < 5; j++){
			...
			gpPoints0[5*i+j] = N[0]*ux[0] + N[1]*ux[1] + N[2]*ux[2] + N[3]*ux[3];
		}
	}
...
void stokesSolver::c2_matrix_element(double x1[4], double x2[4], double uy[4], double c2[4][4]){
	...
	for (int i = 0; i < 5; i++){
		for (int j = 0; j < 5; j++){
			...
			gpPoints0[5*i+j] = N[0]*uy[0] + N[1]*uy[1] + N[2]*uy[2] + N[3]*uy[3];
		}
	}
...
void stokesSolver::C1_matrix(){
	...
	for (int i = 0; i < num_Q1isoQ2_element; i++){
		for (int j = 0; j < 4; j++){
			element_points[0][j] = points[0][quadrilaterals[j][i]];
			element_points[1][j] = points[1][quadrilaterals[j][i]];
			ux[j] = states_array[quadrilaterals[j][i]];
		}
		c1_matrix_element(element_points[0], element_points[1], ux, c1_element);
		for (int m = 0; m < 4; m++){
			for (int n = 0; n < 4; n++){
				MatSetValue(C1, quadrilaterals[m][i], num_node_Q1isoQ2+quadrilaterals[n][i], c1_element[n][m], ADD_VALUES);
			}
		}
	}
	...
	for (int i = 0; i < num_Q1isoQ2_element; i++){
		for (int j = 0; j < 4; j++){
			element_points[0][j] = points[0][quadrilaterals[j][i]];
			element_points[1][j] = points[1][quadrilaterals[j][i]];
			uy[j] = states_array[quadrilaterals[j][i]+num_node_Q1isoQ2];
		}
		c1_matrix_element(element_points[0], element_points[1], uy, c1_element);
		for (int m = 0; m < 4; m++){
			for (int n = 0; n < 4; n++){
				MatSetValue(C1, quadrilaterals[m][i], quadrilaterals[n][i], c1_element[n][m], ADD_VALUES);
			}
		}
	}
...
void stokesSolver::C2_matrix(){
	double c2_element[4][4];
	double element_points[2][4];
	double ux[4];
	double uy[4];
	for (int i = 0; i < num_Q1isoQ2_element; i++){
		...
		c2_matrix_element(element_points[0], element_points[1], ux, c2_element);
		for (int m = 0; m < 4; m++){
			for (int n = 0; n < 4; n++){
				MatSetValue(C2, num_node_Q1isoQ2+quadrilaterals[m][i], num_node_Q1isoQ2+quadrilaterals[n][i], c2_element[n][m], ADD_VALUES);
			}
		}
	}
	...
	for (int i = 0; i < num_Q1isoQ2_element; i++){
		...
		c2_matrix_element(element_points[0], element_points[1], uy, c2_element);
		for (int m = 0; m < 4; m++){
			for (int n = 0; n < 4; n++){
				MatSetValue(C2, num_node_Q1isoQ2+quadrilaterals[m][i], quadrilaterals[n][i], c2_element[n][m], ADD_VALUES);
			}
		}
	}
	MatAssemblyBegin(C2, MAT_FINAL_ASSEMBLY);
	MatAssemblyEnd(C2, MAT_FINAL_ASSEMBLY);	
};
...
void stokesSolver::system_matrix(){
	...
	MatAXPY(A, 1, A1, DIFFERENT_NONZERO_PATTERN);
	MatAXPY(A, 1, C1, DIFFERENT_NONZERO_PATTERN);
	MatAXPY(A, 1, C2, DIFFERENT_NONZERO_PATTERN);
};
...
void stokesSolver::forwardEqn(){
	...
	VecAXPBY(states, 0.01, 0.99, states_n);
	VecCopy(states, states_n);
	VecGetArray(states, &states_array);
	std::copy(states_array, states_array+num_node_Q1isoQ2, u1.get());
	std::copy(states_array+num_node_Q1isoQ2, states_array+2*num_node_Q1isoQ2, u2.get());
...
void stokesSolver::solve()
{
	...
	for (int i = 0; i < 10; ++i){
		updateCmatrix();
		MatMult(finalMatrix, states_n, states_r);
		VecAXPY(states_r, -1, finalRhs);
		VecNorm(states_r, NORM_2, &residual);
...
int main(int argc, char* argv[]){
	...
	if (task == 6){ //Generate Reference
		double quadx[64]; //= {-0.9894, -0.9446, -0.8656, -0.7554, -0.6179, -0.4580, -0.2816, -0.0950,   0.0950,    0.2816,    0.4580,    0.6179,   0.7554,  0.8656,    0.9446,    0.9894};
		double quadw[64]; //= {0.0272, 0.0623, 0.0952, 0.1246, 0.1496, 0.1692, 0.1826, 0.1895, 0.1895,    0.1826,    0.1692,    0.1496,    0.1246, 0.0952,    0.0623,    0.0272};
