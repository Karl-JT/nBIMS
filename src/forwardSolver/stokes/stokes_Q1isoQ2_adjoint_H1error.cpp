...
public:
	...
	void int_vector(double expCoef);
	void system_rhs();
...
void stokesSolver::int_vector_element(double x1[], double x2[], double vx[], double vy[], double expCoef){
	...
	for (int i = 0; i < 4; i++){
		...
		vy[i] = 0;
		for (int k = 0; k < 25; k++){
			vx[i] += gpWeights[k]*gpPoints1x[k]*gpPoints2x[k][i]*Jdet[k];
			vy[i] += gpWeights[k]*gpPoints1y[k]*gpPoints2y[k][i]*Jdet[k];
		}
...
double stokesSolver::solve_2(double obs)
{
	forwardEqn();
	double obsResult = obsOuput();
	double lnlikelihood = -0.5*pow(obsResult-obs, 2);
	return lnlikelihood;
}
...
double stokesSolver::obsOutput(){
		...
		VecDot(refSolver.intVecObs, refSolver.states, &obs);
		obs = 100*obs;
		return obs;
}
...
double stokesSolver::qoiOutput(){
		...
		VecAssemblyBegin(refSolver.states);
		VecAssemblyEnd(refSolver.states);
		VecDot(refSolver.intVecQoi, refSolver.states, &qoi);
		qoi = 100*qoi;
		return qoi;
}
...
void plain_mcmc_2(stokesSolver *solver, int max_samples, double obs, double mean, int coef_size)
{
    std::uniform_real_distribution<double> uni_dist(0.0, 1.0);
    double u[coef_size];
    double u_next[coef_size];
    ...
    for (int i = 0; i < max_samples; i++){
		...
		lnlikelihood_next = solver->solve_2(obs);
...
void ml_mcmc_2(int levels, std::vector<std::shared_ptr<stokesSolver>> solvers, double obs[][4], double out[], int size, int a, int coef_size)
{
    std::uniform_real_distribution<double> uni_dist(0.0, 1.0);
    std::normal_distribution<double> norm_dist(0.0, 1.0);

    // int M = pow(levels, a-2)*pow(2.0, 2.0*levels);
    int M = pow(2.0, 2.0*levels)/pow(levels, 2);
    double mean;
    plain_mcmc_2(solvers[0].get(), M, obs, size, mean, coef_size);
    double L[levels+1][coef_size];
    std::cout << "L0: ";
    for (int m_idx = 0; m_idx < coef_size; ++m_idx){
        L[0][m_idx] = mean[m_idx];
...
//		std::cout << "L8: " << output << std::endl;
	...
	if (task == 6){ //Generate Observation
		...
		refSolver.int_vector(1.5);
		VecDot(refSolver.intVec, refSolver.states, &obs);
		obs = 100*obs;

		refSolver.int_vector(0.5);
		VecDot(refSolver.intVec, refSolver.states, &qoi);
		qoi = 100*qoi;

		localNum += qoi*quadw[2*rank]*exp(-pow(obs+0.8727094065, 2)/2)+(-qoi)*quadw[2*rank]*exp(-pow(-obs+0.8727094065, 2)/2);
		localDen += quadw[2*rank]*exp(-pow(obs+0.8727094065, 2)/2)+quadw[2*rank]*exp(-pow(-obs+0.8727094065, 2)/2);

		std::cout << "rank: " << rank << " obs: " << obs << " qoi: " << qoi << std::endl;

		refSolver.u[0] = quadx[2*rank+1];
		std::cout << "rank: " << rank << " coef: " << refSolver.u[0] << std::endl;
		refSolver.forwardEqn();
		...
		localNum += qoi*quadw[2*rank+1]*exp(-pow(obs+0.8727094065, 2)/2)+(-qoi)*quadw[2*rank+1]*exp(-pow(-obs+0.8727094065, 2)/2);
		localDen += quadw[2*rank+1]*exp(-pow(obs+0.8727094065, 2)/2)+quadw[2*rank+1]*exp(-pow(-obs+0.8727094065, 2)/2);
	...
	if (task == 7){
		stokesSolver refSolver(levels, 1, 1);
		refSolver.u[0] = 0.8399;
		std::cout << refSolver.u[0] << std::endl;
		refSolver.forwardEqn();
